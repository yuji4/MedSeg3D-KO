...
본 프로젝트는 **입력 문제(한국어 질의 → M3D 추론)**와 **출력 문제(M3D 영문 결과 → 자연스러운 한국어)** 두 가지를 모두 해결했습니다.
...
**구현:** Gemini 1.5 Flash 우선, API 키 없으면 GoogleTranslate로 자동 fallback. API 키는  파일로 관리합니다.

근본적인 해결책은 한국어로 학습된 의료 VLM을 사용하는 것이며, 현재 구현은 번역 레이어로 이를 보완합니다.
