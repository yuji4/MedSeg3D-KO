...
초기 GoogleTranslator 사용 시 의료 용어가 음차·왜곡되는 문제가 있었습니다. 현재는 Gemini 1.5 Flash로 의학 판독문 수준의 번역을 달성했으나, 근본적인 해결책은 한국어로 학습된 의료 VLM을 사용하는 것입니다.



**2. VQA 특정 병명 질의 신뢰도**

REG(장기 설명)에서는 "지방침윤 징후"를 찾아내면서 VQA에서는 "없다"고 답하는 불일치가 발생합니다. VQA 템플릿이 질문을 일반화하고, M3D 학습 데이터가 전반적 상태 판단에 편향되어 있기 때문입니다. 특정 이상 소견 확인은 **장기 설명(REG)** 활용을 권장합니다.

**3. 구어체 및 혼합어 처리 미흡**

Layer 2 용어 사전에 없는 표현 입력 시 토큰화 오류가 발생합니다.
...
모든 정량 실험이 5개 케이스, 단일 장기(췌장) 기준으로 수행되었습니다. 통계적 일반화를 위한 대규모 검증이 필요합니다.

**5. 임상 소견과 장기 설명의 정보 차이**

임상 소견은 세그멘테이션 마스크 기반 **부피만** 측정하며, 형태·밀도 이상은 반영되지 않습니다. 두 결과를 함께 참고해야 합니다.
