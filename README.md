...
본 프로젝트는 **입력 문제(한국어 질의 → M3D 추론)**를 해결하는 것을 목표로 했고, Dice 0.000 → 0.5515로 이를 달성했습니다. 그러나 **출력 문제(M3D 영문 결과 → 자연스러운 한국어)**는 미완성 과제로 남아 있습니다.

### 1. 출력 번역 품질 (핵심 미해결 과제)

M3D가 생성한 영문 의료 텍스트를 GoogleTranslator로 번역하면 의료 용어가 어색하게 변환되거나 문맥이 뭉개집니다.

```
M3D 출력:   "The pancreatic duct is dilated, suggesting possible obstruction."
현재 번역:  "췌관이 팽창되어 가능한 폐색을 시사합니다."     ← 어색
자연스러운: "췌관이 확장되어 있어 폐색 가능성이 있습니다."
```

근본적인 해결책은 한국어로 학습된 의료 VLM을 사용하는 것이지만, 현실적인 대안은 아래와 같습니다.

| 방법 | 기대 효과 | 비고 |
|------|---------|------|
| 의료 용어 후처리 규칙 추가 | 빈출 오역 패턴 교정 | 무료, 커버리지 제한 |
| 의료 특화 번역 모델 도입 | 전반적 품질 향상 | 추가 모델 필요 |
| LLM API 활용 (번역 전용) | 가장 자연스러운 결과 | API 비용 발생 |
...
모든 정량 실험이 5개 케이스, 단일 장기(췌장) 기준으로 수행되었습니다. M3D의 추론 비용(케이스당 약 30초)으로 인한 현실적 제약으로, 다른 장기로의 일반화 가능성은 추가 검증이 필요합니다.
