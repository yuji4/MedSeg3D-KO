...
VQA, 소견 생성, 영역 설명의 모델 출력(영문)을 GoogleTranslator로 한국어로 번역하는 과정에서 의료 용어가 어색하게 번역되거나 문맥이 뭉개지는 경우가 있습니다.

```
모델 출력: "The pancreatic duct is dilated, suggesting possible obstruction."
현재 번역: "췌관이 팽창되어 가능한 폐색을 시사합니다."   ← 어색
이상적:   "췌관이 확장되어 있어 폐색 가능성이 있습니다."
```

의료 전문 번역 모델로 교체하거나, 자주 오역되는 패턴을 후처리 규칙으로 보정하면 개선 가능합니다.

### 구어체 및 혼합어 처리 미흡

Layer 2 용어 사전에 등록된 표현만 인식합니다. 사전에 없는 구어체나 한영 혼합어 입력 시 토큰화 오류가 발생합니다.

| 입력 | 처리 결과 |
|------|---------|
| "이자 분할해줘" | Layer 2 미매칭 → 오류 |
| "pancreas 분할해줘" | 혼합 토큰화 오류 |
| "왼쪽 콩팥 보여줘" | 일부 변형 미지원 |

용어 사전 확충 및 전처리 단계에서 혼합어 분리 로직 추가가 필요합니다.

### 실험 규모의 한계

모든 정량 실험이 5개 케이스, 단일 장기(췌장)를 기준으로 수행되었습니다. M3D의 높은 추론 비용(케이스당 약 30초)으로 인한 현실적 제약이었으나, 통계적 신뢰성과 다른 장기로의 일반화 가능성은 추가 검증이 필요합니다.

### 모델 자체 한계

M3D-LaMed는 동일 입력에 대해 추론마다 결과가 약간씩 달라질 수 있습니다. 또한 3D 볼륨을 32×256×256으로 고정 리사이즈하는 과정에서 얇거나 작은 구조물의 세부 정보가 손실될 수 있습니다.

### Layer 1 경계 표현 오분류

규칙 기반 분류기는 의도가 경계에 걸친 표현에서 오분류합니다. (실험 5 기준 4건/50건)

| 오분류 예시 | 원인 |
|------------|------|
| "췌장만 보고싶어" → VQA | SEG 키워드 없음 |
| "이상 소견 있나?" → Report | "소견" 키워드 충돌 |

ML 기반 분류기(예: 경량 fine-tuned BERT) 도입으로 개선 가능합니다.
